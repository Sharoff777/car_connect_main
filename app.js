    ...
    <div class="form-footer-bar">
      ...
      function submitSellForm() {
        ...
        document.getElementById("err-mileage").textContent = mileage && parseInt(mileage) >= 0 ? "" : "Enter valid mileage";

        if (!name || !phone || !make || !model || !mileage || parseInt(mileage) < 0) return;
        ...
        if (fileInput && fileInput.files.length > 0) {
          Array.from(fileInput.files).slice(0, 10).forEach((file, i) => {
        ...
        fetch("mail.php", { method: "POST", body: fd })
          .then(res => res.text())
          .then(() => {
            renderSellForm(true);
          })
          .catch(() => {
            if (btn) { btn.disabled = false; btn.innerHTML = "Submit via WhatsApp"; }
