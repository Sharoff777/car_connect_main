...
function vehicleCardHTML(v) {
  const badge = v.badge
    ...
    : "";
  const conditionCls =
    v.condition === "Brand New" ? "vehicle-cond-new" : "vehicle-cond-used";
    ...
    <div class="form-body-inner">
      ...
      <div class="form-row" style="margin-top:14px">
        ...
        <div class="form-group" style="margin-bottom:0">
          ...
          <input class="form-control" type="number" id="f-year" placeholder="e.g. 2019" min="1990" max="2026"/>
        </div>
      </div>
      <div class="form-row" style="margin-top:14px">
        <div class="form-group" style="margin-bottom:0">
          <label>Asking Price (LKR) <span style="color:var(--g300);font-weight:400">— optional</span></label>
          <input class="form-control" id="f-price-ask" placeholder="e.g. 18,500,000"/>
        </div>
        <div class="form-group" style="margin-bottom:0">
          <label>Location</label>
          <input class="form-control" id="f-location" placeholder="e.g. Colombo, Kandy"/>
        </div>
      </div>
      <div class="form-group" style="margin-top:14px">
        <label>Additional Notes <span style="color:var(--g300);font-weight:400">— optional</span></label>
        <input class="form-control" id="f-notes" placeholder="Condition, extras, urgency…"/>
      </div>
...
function submitSellForm() {
  ...
  const price    = document.getElementById("f-price-ask").value.trim();
  ...
  // Build FormData — text fields + every uploaded photo
  const fd = new FormData();
  fd.append("username", name);
  fd.append("phone",    phone);
  fd.append("car_make", make);
  ...
  if (fileInput && fileInput.files.length > 0) {
    Array.from(fileInput.files)
      .slice(0, 10)
      .forEach((file, i) => {
        fd.append("filename" + (i + 1), file, file.name);
      });
  }
  ...
  fetch("/api/sell", { method: "POST", body: fd })
    .then((res) => res.json())
    .then((data) => {
      if (data.ok) {
        renderSellForm(true);
